...
- Findings reporting: whenever a plan or task uncovers anything that hurts the project (bug risk, security issue, performance problem, broken workflow) or would improve efficiency (reuse, deletion, simpler approach), report it to the user at the end of the task with a final verdict — one line per finding: what it is, why it matters, and the recommended action. The verdict is part of the deliverable, not an afterthought.
