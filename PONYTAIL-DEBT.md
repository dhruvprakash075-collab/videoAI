...
Harvested 2026-07-31. One row per `ponytail:` marker. `no-trigger` rows name
...
| core/outline_shaping.py:107 | Positional aliases kept at max weight | Heuristic until stable IDs exist | Stable alias IDs available |
...
| core/segment_runner.py:391 | Checkpoints predating the engine field lack it | Old checkpoints untyped | Checkpoint schema bump |
| core/segment_runner.py:1018 | memory_review loads Ollama in same phase as render | Sequential phase overlap | Phase pipelining |
