...
def test_chat_returns_text_on_success():
    client = _make_client()
    chat_resp = MagicMock()
    chat_resp.read.return_value = json.dumps(
...
def test_director_call_ollama_delegates_to_client():
    ...
    agent = DirectorAgent(llm_config={
        "ollama": {"host": "http://localhost:11434", "request_timeout": 10,
                   "breaker_fails": 3, "breaker_cooldown_s": 30},
...
def test_director_call_ollama_returns_empty_on_breaker_open():
    ...
    agent = DirectorAgent(llm_config={
        "ollama": {"host": "http://localhost:11434", "request_timeout": 10,
                   "breaker_fails": 1, "breaker_cooldown_s": 60},
        "models": {"director": "test-director"},
    })
...
def test_translate_hinglish_delegates_and_records_seg_on_failure():
    ...
    assert any(d["seg"] == 7 and d["stage"] == "translation_fallback"
               for d in UIState.degradations)
    reset_ollama_client()
...
def test_specialized_models_routes_through_breaker():
    ...
    with patch("utils.ollama_client.get_ollama_client", return_value=_StubClient()):
        result = _call_ollama("hi there", "script-reviewer", format_json=True,
