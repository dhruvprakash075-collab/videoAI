...
_LATIN_HEAVY  = ("यह " + "Latin text here " * 8 +
                 "और यह भी देवनागरी है।")  # ~10 Deva chars, mostly Latin
...
def test_retranslate_triggers_on_latin_heavy():
    ...
    with patch.object(agent, "_call_ollama_chat",
                      side_effect=[_LATIN_HEAVY, _CLEAN_DEVA]) as mock_llm:
        result = agent.translate_to_devanagari("Hello world.", plan)
...
def test_retry_cap_respected():
    ...
    with patch.object(agent, "_call_ollama_chat",
                      return_value=_LATIN_HEAVY) as mock_llm:
        result = agent.translate_to_devanagari("Hello world.", plan)
...
def test_best_result_kept():
    ...
    with patch.object(agent, "_call_ollama_chat",
                      side_effect=[_LATIN_HEAVY, _medium, _LATIN_HEAVY]):
        result = agent.translate_to_devanagari("Hello world.", plan)
