...
def test_tts_generate_calls_f5_when_engine_is_f5(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ap, "_get_config", lambda: {
...
def test_tts_generate_f5_failure_falls_back_to_omnivoice(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ap, "_call_f5_worker",
                        lambda *a, **kw: {"status": "error", "message": "F5 not installed"})

    omni_called = []

    def _fake_omni(text, lang="hi", output_dir=None, voice_sample="",
                   speed_override=None, sentence_gap_ms=None):
...
def test_tts_generate_f5_and_omni_failure_falls_back_to_edge(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ap, "_call_f5_worker",
                        lambda *a, **kw: {"status": "error", "message": "no F5"})
    monkeypatch.setattr(ap, "_call_omnivoice_worker",
                        lambda *a, **kw: {"status": "error", "message": "no omni"})
