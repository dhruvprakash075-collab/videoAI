...
def t_token_budget_identity_heavy():
    ...
    cfg = _alt_config()
    cfg["visual"] = {
        "style": "cinematic realism, dramatic lighting, 8k, ultra detailed, volumetric fog"
    }
    cfg["characters"] = {
        "hero": {
            "name": "The Hero",
    ...
    result, _neg = enrich_prompts(
        "the hero raises the sword against the storm", "The hero stood firm.", cfg, plan
    )
...
def t_loudnorm_alt_target():
    ...
    calls = []
    fake = (
        '{"input_i":"-22","input_tp":"-3","input_lra":"9","input_thresh":"-32","target_offset":"0"}'
    )
...
def t_edge_english_path():
    ...
    with (
        patch("audio.audio_proxy.load_config", return_value=cfg),
...
def t_world_state_english():
    ...
    cfg = _alt_config()
    script = (
        "Captain Reyes betrayed the crew. The old map could only be read under "
...
def t_staged_batches():
    ...
    n_segs = 5
    bs = max(1, lookahead)
    batches = [list(range(1, n_segs + 1))[k : k + bs] for k in range(0, n_segs, bs)]
