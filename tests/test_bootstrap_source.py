...
def _doc(text, source_type="md", language="en", metadata=None, word_count=None):
    ...
    return SourceDocument(
        text=text, word_count=word_count, language=language,
...
class TestLoadSplitFilePath:
    def test_md_file_loads_and_splits(self, tmp_path):
        f = tmp_path / "test.md"
        f.write_text("# Title\n\nBody text here.\n", encoding="utf-8")
        doc = _doc("# Title\n\nBody text here.\n", source_type="md", word_count=5,
                   metadata={"title": "Title"})
        with patch("utils.source_loader.load_source", return_value=doc) as mock_load, \
             patch("utils.source_splitter.split_source", return_value=_chunks(1)) as mock_split:
            chunks, topic, content = _load_and_split_source(str(f), _args(), {"script": {"words_per_segment": 100}})
    ...
    def test_filename_stem_used_as_topic(self, tmp_path):
        ...
        with patch("utils.source_loader.load_source", return_value=doc), \
...
class TestLoadSplitUrl:
    def test_url_loads(self):
        doc = _doc("Article body", source_type="url", word_count=2,
    ...
    def test_url_falls_back_to_url(self):
        doc = _doc("Body", source_type="url", word_count=1,
...
class TestSegmentCountDerivation:
    ...
    def test_segment_count_override_used(self, tmp_path):
        ...
        with patch("utils.source_loader.load_source", return_value=doc), \
            ...
            _load_and_split_source(str(f), _args(segment_count=10), {"script": {"words_per_segment": 100}})
        args = mock_split.call_args.args
        assert args[1] == 10
    ...
    def test_words_per_segment_override(self, tmp_path):
        ...
        with patch("utils.source_loader.load_source", return_value=doc), \
            ...
            _load_and_split_source(str(f), _args(words_per_segment=50), {"script": {"words_per_segment": 100}})
        args = mock_split.call_args.args
        assert args[1] == 4
...
class TestEndToEndMock:
    def test_source_chunks_passed_to_run_long_pipeline(self, tmp_path):
        ...
        with patch("utils.source_loader.load_source", return_value=doc), \
            ...
            chunks, topic, content = _load_and_split_source(str(f), _args(), {"script": {"words_per_segment": 100}})
            mock_rlp(topic=topic, project_name=None, resume=True, skip_rvc=False,
                     dry_run=True, duration_min=None, director_mode=False, series_mode=False,
                     content_text=content, preview_mode=False, words_per_segment=None,
                     images_per_segment=None, segment_count=None, source_chunks=chunks)
