...
def test_vram_always_low_proceeds_with_warning(caplog):
    ...
    with patch("torch.cuda.is_available", return_value=True), \
         ...
         patch("torch.cuda.empty_cache"), \
         patch("time.sleep"), \
         patch("time.time", side_effect=[0.0, 100.0, 100.0, 100.0]):  # instant timeout
        with caplog.at_level(logging.WARNING):
            _evict_ollama_models(_make_config(wait_s=0.001), reason="test")

    # Should have logged a warning about low VRAM
    assert any("VRAM" in r.message or "low" in r.message.lower()
               for r in caplog.records), "Expected VRAM warning in logs"
