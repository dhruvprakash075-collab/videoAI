...
class VisionCache:
    def __init__(self, cache_dir="cache",
                 config_path=None,
                 prompts_path="prompts.yaml", prompt_key="analyze_story",
                 force_refresh=False, max_entries=200):
        self._dir = Path(cache_dir); self._dir.mkdir(parents=True, exist_ok=True)
        ...
        self._data = {}; self._meta_data = {}
        self._cfg_hash_cache = None; self._prp_hash_cache = None
        self._cfg_mtime = 0; self._prp_mtime = 0
        for p in [self._cache, self._meta]:
            if p.exists():
                try: d = json.loads(p.read_text(encoding="utf-8"))
                except (json.JSONDecodeError, OSError): d = {}
                if p == self._cache: self._data = d
                else: self._meta_data = d
    ...
    def _config_hash(self):
        ...
        with open(self._cfg_path, encoding='utf-8') as f: c = yaml.safe_load(f) or {}
        r = {"models":c.get("models",{}),"ollama_host":c.get("ollama",{}).get("host"),
             "tts_lang":c.get("tts",{}).get("lang")}
        self._cfg_hash_cache = hashlib.sha256(json.dumps(r,sort_keys=True).encode()).hexdigest()[:16]
        self._cfg_mtime = mtime
        return self._cfg_hash_cache

    def _prompt_hash(self):
        if not self._prp_path or not self._prp_path.exists(): return "noprompts"
        mtime = self._prp_path.stat().st_mtime
        if self._prp_hash_cache and mtime == self._prp_mtime:
            return self._prp_hash_cache
        with open(self._prp_path, encoding="utf-8") as f: p = yaml.safe_load(f) or {}
    ...
    def set(self, topic, vision_doc, content_text: str = ""):
        with self._lock:
            ...
            self._meta_data[k] = {"cache_version":CACHE_VERSION,
                                  "created_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),
                                  "topic":topic[:60]}
            if len(self._data) > self._max:
                oldest = sorted(self._meta_data,
                               key=lambda x: self._meta_data[x].get("created_at",""))[:max(1,self._max//5)]
                for ok in oldest: self._data.pop(ok,None); self._meta_data.pop(ok,None)
            try:
                tmp_cache = self._cache.with_suffix(".tmp")
                tmp_meta = self._meta.with_suffix(".tmp")
                tmp_cache.write_text(json.dumps(self._data,indent=2,ensure_ascii=False),encoding="utf-8")
                tmp_meta.write_text(json.dumps(self._meta_data,indent=2,ensure_ascii=False),encoding="utf-8")
                import os
                os.replace(str(tmp_cache), str(self._cache))
                os.replace(str(tmp_meta), str(self._meta))
            except Exception as e: log.warning(f"VisionCache save failed: {e}")
