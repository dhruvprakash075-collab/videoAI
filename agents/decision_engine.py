...
def _apply_director_proposals(rec, vision_doc: dict, config: dict) -> None:
    ...
    if rec_dur and rec_dur > 0:
        rec.set("total_duration_min", float(rec_dur), "director",
                rationale="Director content analysis")

    # Segment count from vision doc (if present)
    seg_count = vision_doc.get("segment_count", 0)
    if seg_count and seg_count > 0:
        rec.set("segment_count", int(seg_count), "director",
                rationale="Director story analysis")

    # Words per segment from vision doc
    wps = vision_doc.get("words_per_segment", 0)
    if wps and wps > 0:
        rec.set("words_per_segment", int(wps), "director",
                rationale="Director pacing analysis")

    # Images per segment from vision doc
    ips = vision_doc.get("image_count_per_segment", 0)
    if ips and ips > 0:
        rec.set("images_per_segment", int(ips), "director",
                rationale="Director visual analysis")
...
def _apply_writer_input(rec, writer_input: dict) -> None:
    ...
    for w_key, r_field in mapping.items():
        ...
        if val and isinstance(val, (int, float)) and val > 0:
            applied = rec.set(
                r_field, int(val), "writer",
                rationale=rationale_base
            )
            if applied:
                log.info(
                    f"[DECISION ENGINE] Writer adjusted '{r_field}' → {int(val)}"
                )
            else:
                log.debug(
...
def _apply_user_locks(rec, user_locks: dict, cli_flags: dict) -> None:
    ...
    for flag, field in cli_map.items():
        ...
        if val is not None:
            ...
            rec.set(field, val, "cli_flag", lock=True,
                    rationale=f"CLI flag {_flag_label}")
    ...
    for u_key, field in user_map.items():
        ...
        if val is not None:
            rec.set(field, val, "user", lock=True,
                    rationale="User explicit override")
    ...
    if run_mode in ("project", "one_time"):
        rec.set("run_mode", run_mode, "user" if "run_mode" in user_locks else "cli_flag",
                lock=True)
