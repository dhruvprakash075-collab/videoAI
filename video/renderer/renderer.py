...
def build_html(audio_path, image_paths, script, duration,
               subtitle_script=None, transition="cross_fade",
    ...
    parts.append('<div id="root" data-composition-id="main" data-width="1920" '
                 f'data-height="1080" data-start="0" data-duration="{duration:.1f}">')
    aname = audio_path.name if audio_path else "narration.wav"
    parts.append(f'<audio id="main-audio" src="assets/{aname}" data-start="0" '
                 f'data-duration="{duration:.1f}" data-track-index="0" data-volume="1"></audio>')
    for i, img in enumerate(image_paths):
        parts.append(f'<img id="scene-{i}" class="clip" src="assets/{Path(img).name}" '
    ...
    sentences = [s.strip() for s in re.split(r"(?<!\d)\.(?=\s|$)|[!?।]+", caption_text) if s.strip()]
    if not sentences:
            ...
            try:
                ...
                if word_data and lines:
                    ...
                    if all_words:
                        ...
                        log.debug(f"[build_html] Using word timestamps for {len(timings)} caption lines")
    ...
    for i, (line, (t_start, t_dur)) in enumerate(zip(lines, caption_timings, strict=False)):
        ...
        parts.append(f'<div id="cap-{i}" class="clip" data-start="{t_start:.3f}" '
    ...
    parts.append('</div>')
    parts.append('<script>window.__timelines=window.__timelines||{};'
                 'window.__timelines["main"]={seek:function(){}, pause:function(){}, duration:function(){return ' + str(duration) + ';}};</script>')
    parts.append('</body></html>')
    return '\n'.join(parts)


def render_with_assets(compositions_dir, output_path, audio_path, image_paths,
                       script, style="", html_content=None, transition="cross_fade",
                       subtitle_script=None, word_timestamps_json=None, is_final=True):
    ...
    if _use_hyperframes:
        try:
            ...
            if html_content:
                ...
                html_text = build_html(audio_path, image_paths, script, dur,
            ...
            try:
                ...
                UIState.add_degradation(_seg_num_deg, "hyperframes_fallback",
                                        f"Hyperframes failed: {str(hf_err)[:100]}")
